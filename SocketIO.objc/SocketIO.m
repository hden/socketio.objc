...
static NSString* kHandshakeURL = @"%@://%@%@/%@/?transport=polling&b64=0&t=%.0f%@";
...
- (void) connectToURL:(NSURL *)url withParams:(NSDictionary *)params
{
    ...
    [self connectToHost:url.host
                 onPort:url.port.longLongValue
             withParams:params
          withNamespace:url.path
...
{
    NSMutableDictionary *payload = [NSMutableDictionary dictionaryWithObject:topic forKey:@"name"];
    
    // do not require arguments
    if (args != nil) {
        if ([args isKindOfClass:[NSArray class]]) {
            [payload setObject:args forKey:@"args"];
        } else {
            [payload setObject:[NSArray arrayWithObject:args] forKey:@"args"];
        }
    }
    
    SocketIOPacket *packet = [[SocketIOPacket alloc] initWithType:@"event"];
    packet.data = [SocketIOJSONSerialization JSONStringFromObject:payload error:nil];
    packet.pId  = [self addAcknowledge:cb];
...
//- (void) emit:(NSString *)topic withArgs:(id)arg, ... NS_REQUIRES_NIL_TERMINATION
//{
//    NSMutableArray *arguments = [[NSMutableArray alloc] init];
//    SocketIOCallback cb = nil;
//    
//    // va_list
//    va_list args;
//    va_start(args, arg);
//    for (id el = arg; el != nil; el = va_arg(args, id)) {
//        [arguments addObject:el];
//    }
//    va_end(args);
//    
//    if ([[arguments lastObject] isKindOfClass:NSClassFromString(@"NSBLock")]) {
//        cb = [arguments lastObject];
//        [arguments removeLastObject];
//    }
//    [self emit:topic withArgs:arguments andCallback:cb];
//}
...
- (NSArray *) deframe:(NSString *)str
{
    ...
    NSNumber *type     = [NSNumber numberWithInt:[[frame substringToIndex:1] intValue]];
    NSData   *fd       = [[frame substringFromIndex:1] dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary *data = [NSJSONSerialization JSONObjectWithData:fd options:NSJSONReadingAllowFragments error:nil];
    
    if (data) {
        NSMutableDictionary *packet = [NSMutableDictionary dictionaryWithObject:type forKey:@"type"];
        [packet setObject:data forKey:@"data"];
        
        [frames addObject:packet];
    }
    
    if (body.length > frame.length) {
        // more frames
        [frames addObjectsFromArray:[self deframe:[body substringFromIndex:idx]]];
    }
...
- (void) connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    NSArray *frames = [self deframe:responseString];
    NSDictionary *data = frames[0][@"data"];
    ...
    if (_sid.length < 1) {
        ...
        if ([_sid rangeOfString:@"error"].location != NSNotFound || [regexTest evaluateWithObject:_sid]) {
            [self connectToHost:_host
                         onPort:_port
                     withParams:_params
                  withNamespace:_endpoint
          withConnectionTimeout:defaultConnectionTimeout];
            return;
        }
        
        // check heartbeat timeout
        _heartbeatTimeout = [(NSNumber *)data[@"pingTimeout"] doubleValue];
        ...
        // get transports
        NSArray *transports = data[@"upgrades"];
